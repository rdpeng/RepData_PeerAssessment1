...
```{r, echo=TRUE}
hist(total$Steps,main="Histogram of number of steps taken per day",xlab="Total steps taken per day", col="skyblue")
...
### Time-series plot of the average steps taken per day in a 5 minute interval
...
2. Which 5 minute interval accross all days contains the maximum number of steps
...
```{r}
par(mfrow = c(1,2))
hist(new, col = "pink",xlim=c(0,25000),ylim = c(0,40),main = paste("Histogram (Imputed)"),xlab= "Total steps taken per day",adj=0.5)
...
#Great input here from the ifelse conditions which very simply shows me how to tackle this matter, by using mutate to generate this extra column)
#ifelse(test, yes, no)
...
# double checked with grep('weekend',impute$weektype) if it was working 

# we need now to summarize once more so we can plot the time series 
week<-aggregate(impute$steps,by=list(impute$weektype),FUN=mean,na.rm=TRUE)
colnames(week)<-c("weektype","steps")
p<-ggplot(week,aes(x=steps))+geom_line()+ facet_grid(~weektype)
