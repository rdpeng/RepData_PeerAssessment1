...
The mean of the steps is **`r steps.mean.per.day`** and the median is **`r steps.median.per.day`**.
...
The interval **`r max.item$interval`** contains **`r max.item$steps`** steps, which is the most.
...
We can see that there are **`r steps.na`** missing in steps column, **`r date.na`** in date column and **`r interval.na`** in interval column. Missing values only appear in the steps column. So in this case, we can use the **mean value of the same interval** to impute the missing value.
...
The mean of the steps is **`r steps.imputed.mean.per.day`** and the median is **`r steps.imputed.median.per.day`**. Compared with the original data, the mean and median value is basically the same.
