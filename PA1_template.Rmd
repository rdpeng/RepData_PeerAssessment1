...
A data of steps given by a person, measured at 5 minutes interval across the day, was registred for Octover and November of 2012,  with a Nike Fuelband device and a samsung galaxy S4 with android 4.1 cellphone.
...
The total, mean and median of the steps was calculater per each day.
```{r,results='hide'}
...
A histogram of the steps per interval, considering all days, is shown below:
...
The data was gruoped per interval, across all days, and their mean, median and total steps were calculated.
```{r,results='hide'}
## INTERVAL ANALYSIS
...
The interval with the max steps in average is:
```{r}
...
Considering the total the raw data, the NA values represent a percentaje of (expressed as decimal):
```{r,results='asis'}
...
```{r}
data.frame(crudos[280:300,],RD[280:300,],Int.mena=c(ID$mean[280:288],ID$mean[1:12]))
```
From left to right, the columnos 1, 2 and 3 represent the original data. Columns 4, 5 and 6 represent the replaced data, and the last one show the mean for the ith interval.

The data replaced was grouped per day, as previus analysis.
```{r,results='hide'}
...
The scenario for each day, with replaced data, es shown below:
```{r}
RDD
```
...
An analysis of the steps per interval was separated in days of the week and weekend.
```{r,results='hide'}
...
A plot comparing both patterns is shown below: 
```{r fig.width=7, fig.height=6}
par(mfrow=c(2,1))
