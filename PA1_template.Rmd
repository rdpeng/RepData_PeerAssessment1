...
require(knitr) # without this line knitting haults with "object 'opts_chunk' not found"
...
```
The **mean total** and **median total** **steps taken per day** are, respectively, **`r me`** and **`r md`**.
...
```
The answer also turns out to be **`r count_na_2`** rows so our previous result is valid.
...
```
**`r format(me.i, scientific = F)`** and **`r format(md.i, scientific = F)` ** respectively compared to **`r format(me, scientific = F)`** and **`r format(md, scientific = F)`**.
